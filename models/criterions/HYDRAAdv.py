...
class HYDRAAdv(HYDRA):
    def __init__(self, arguments, local_pruning, *args, **kwargs):
        super(HYDRAAdv, self).__init__(arguments, local_pruning, *args, **kwargs)
        self.adv_loss = torch.nn.KLDivLoss(reduction='batchmean')
    ...
    def prune(self, percentage=0.0, **kwargs):
        ...
        for batch in self.test_loader:
            if not self.local_pruning:
                self.handle_global_pruning(self.percentage)
            x, y = batch
        ...
        for i in range(self.arguments["epochs"]):
            ...
            # Validate on attacks
            for batch in self.val_loader_unnormalized:
                if not self.local_pruning:
                    self.handle_global_pruning(percentage)
                x, y = batch
                x, y = x.to(self.device).float(), y.to(self.device)

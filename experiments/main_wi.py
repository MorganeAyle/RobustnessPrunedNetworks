...
from common import log_start_run, assert_compatibilities
...
def main(
    ...
    checkpoints = os.listdir(os.path.join(DATA_MANAGER.directory, RESULTS_DIR))
    ...
    path = os.path.join(DATA_MANAGER.directory, RESULTS_DIR, checkpoint_path, MODELS_DIR,
                        arguments["checkpoint_model"])
    load_checkpoint_wi(path, model, out)

    backup_model = deepcopy(model)

    # load augerino weights
    width = torch.load("./data/augmentations.pt")
    width = width.cpu()
    ...
    #         # Evaluate only on corruptions with severity 5
...
def load_checkpoint_wi(path, model, out):
